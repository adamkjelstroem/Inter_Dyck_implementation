...
//				(isPaddle ? ",bdd:true,backend:javabdd,context:objsens" : "")
				...
				"-cp", liargs[0] + java.io.File.pathSeparator + liargs[1], "-f", (true?"J":"n")
				,"-allow-phantom-refs"
				, "-process-dir", liargs[1],
				"-main-class", liargs[2], liargs[2], };

		// for (int i = 0; i < args.length; i++) {
		// 	System.out.println(args[i] + "   ");
		// }
		System.out.println("Soot called with main class : "+liargs[2]);
	...
	protected void internalTransform(String phaseName, Map options) {
		...
		if (/*Util.MEASURE_SPARK*/false) {
			...
			System.out.printf("[6e9waprU][internalTransform] %s & %d & %d (%f\\%%)\n", Util.BENCH_NAME, total, lib, ratio * 100);
			
			System.exit(0);
		}
		
		if (/*Util.TEMP_HACK*/false) {
			int numAppMtds = 0;
			int numLibMtds = 0;
		...
		//<NOTE> here is the line of importance
		...
		for (Map.Entry<SootField, HashSet<Pair<DefinitionStmt, SootMethod>>> entry : writes.entrySet()) {
			...
			for (Pair<DefinitionStmt, SootMethod> p1 : writeSet) {
				...
				for (Pair<DefinitionStmt, SootMethod> p2 : writeSet) {
					...
					if (res) {
						if (Util.DEBUG) {
							System.out.println("[race][DEBUG,] " + s1 + "@" + m1 + ", " + s2 + "@" + m2);
